...
<div class="card" style="margin-bottom:1.25rem; padding:1rem;">
    <div style="display:flex; gap:1rem; flex-wrap:wrap; align-items:center;">
        <input type="text" id="filter-search" class="form-input" placeholder="Search by name, IP, MAC, or user..." style="flex:1; min-width:240px; padding:0.5rem 0.75rem; background:var(--bg-input); border:1px solid var(--border-color); border-radius:var(--radius-md); color:#fff; font-size:0.8rem;">
...
<div id="modal-add-device" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-header">
            <h3 style="font-size:1.1rem; font-weight:700; color:#fff;">Register New Network Asset</h3>
            ...
            <div class="modal-body">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                    <div>
                        ...
                        <input type="text" id="dev-ip" class="form-input" required placeholder="192.168.10.45" style="width:100%; padding:0.55rem; background:var(--bg-input); border:1px solid var(--border-color); border-radius:var(--radius-md); color:#fff; font-size:0.8rem;">
                    </div>
                    <div>
                        <label class="form-label" style="display:block; font-size:0.8rem; color:var(--text-muted); margin-bottom:0.3rem;">MAC Address</label>
                        <input type="text" id="dev-mac" class="form-input" required placeholder="00:50:56:AB:CD:EF" style="width:100%; padding:0.55rem; background:var(--bg-input); border:1px solid var(--border-color); border-radius:var(--radius-md); color:#fff; font-size:0.8rem;">
                    </div>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem;">
                    <div>
...
<script type="module">
    ...
    // Parse URL params for pre-filled search
            ...
            tbody.innerHTML = items.map(d => {
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="font-size:0.75rem; color:var(--text-muted); font-family:monospace;">${d.mac_address}</div>
                    </td>
                    <td>
                        <div style="text-transform:capitalize; font-weight:500;">${d.device_type}</div>
                        <div style="font-size:0.75rem; color:var(--text-muted);">${d.operating_system || 'Generic OS'}</div>
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <span style="font-size:0.75rem; color:var(--text-muted);">(${d.risk_level || 'LOW'})</span>
                    </td>
                    <td>
                        <div style="color:var(--text-primary);">${d.assigned_user || 'Unassigned'}</div>
                        <div style="font-size:0.75rem; color:var(--text-muted);">${d.assigned_email || ''}</div>
                    </td>
                    <td>
