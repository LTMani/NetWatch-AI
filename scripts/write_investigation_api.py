...
from app.models.risk import DeviceRiskFactor
...
def investigate_ip(ip_address):
    ...
    if device:
        ...
        avg_loss = (sum(f.packet_loss_pct for f in flows) / len(flows)) if flows else 0.0
        ...
        risk_factors = DeviceRiskFactor.query.filter_by(device_id=device.id).order_by(DeviceRiskFactor.calculated_at.desc()).limit(10).all()
        ...
        data = {
            ...
            'summary': {
                ...
                'last_seen': device.last_seen.isoformat() if device.last_seen else utc_now().isoformat(),
            ...
            'overview': {
                ...
                'location': site.location if site else 'Server Room / LAN',
...
with open('app/routes/investigation_api.py', 'w', encoding='utf-8') as f:
    f.write(code)

print('[+] Created app/routes/investigation_api.py')
